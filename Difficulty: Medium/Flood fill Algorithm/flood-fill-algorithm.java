...
class Solution {
    private void dfs(int row , int col , int image[][] , int newColor , int ans[][] , int initialColor){
        int n = image.length ;
        ...
        for(int i = 0 ; i <  4 ; i++){
            ...
            if(newRow >= 0 && newRow < n && newCol >= 0 && newCol < m && image[newRow][newCol] == initialColor && ans[newRow][newCol] != newColor){
                dfs(newRow , newCol , image , newColor , ans , initialColor); 
            }
        }
    }
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        ...
        dfs(sr , sc , image , newColor , ans , initialColor); 
        return ans ; 
    }
}